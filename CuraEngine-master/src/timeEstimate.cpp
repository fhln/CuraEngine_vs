...
template<typename T> const  T square(T& a) { return a * a; }
...
void TimeEstimateCalculator::plan(Position newPos, double feedrate)
{
    ...
    {
		double xy_jerk = sqrt((current_feedrate[X_AXIS] - previous_feedrate[X_AXIS])*(current_feedrate[X_AXIS] - previous_feedrate[X_AXIS]) + (current_feedrate[Y_AXIS] - previous_feedrate[Y_AXIS])*(current_feedrate[Y_AXIS] - previous_feedrate[Y_AXIS]));
