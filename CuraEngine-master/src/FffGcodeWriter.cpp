...
void FffGcodeWriter::writeGCode(SliceDataStorage& storage, TimeKeeper& timeKeeper)
{
    ...
    setConfigRetraction(storage); //配置storage.retraction_config
    ...
    bool has_raft = getSettingAsPlatformAdhesion("adhesion_type") == Adhesion_Raft; //支持物
    if (has_raft)
    {
...
/*! 
* 打印第一个object前的准备工作，选择起始打印温度，不同打印机的预处理代码
*/
...
void FffGcodeWriter::processNextPrintObjectCode(SliceDataStorage& storage)
{
    ...
    gcode.writeMove(Point(storage.model_min.x, storage.model_min.y), getSettingInMillimetersPerSecond("speed_travel"), 0); //运动到角落点
}
...
{
    int z = getSettingInMicrons("layer_height_0") + layer_nr * getSettingInMicrons("layer_height");
    if (has_raft)
    {
