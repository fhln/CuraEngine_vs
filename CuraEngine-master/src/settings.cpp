...
//c++11 no longer defines M_PI, so add our own  ant.
#ifndef M_PI
...
double SettingsBase::getSettingInPercentage(std::string key)
{
	...
	return 0.0 > atof(value.c_str()) ? 0.0 : atof(value.c_str());
}
...
EGCodeFlavor SettingsBase::getSettingAsGCodeFlavor(std::string key) //!< 返回打印机类型，即G代码所属类型
{
    std::string value = getSettingString(key);
    if (value == "RepRap")
